...
function PositionCoder.formatOutput(element, x, y, parentW, parentH)
	...
	if exists(element) then
		...
		curx, cury = guiGetPosition(element, false)
	...
	return string.format("local parentW, parentH = %s, %s; if parentW == -1 then parentW, parentH = guiGetScreenSize() end; local width, height = %s, %s; local curx, cury = %s, %s; return %s", pW, pH, w, h, curx, cury, x),
			string.format("local parentW, parentH = %s, %s; if parentW == -1 then parentW, parentH = guiGetScreenSize() end; local width, height = %s, %s; local curx, cury = %s, %s; return %s", pW, pH, w, h, curx, cury, y)
end
