...
function play ()
	playSFX("script", 198, 3, false)
end
addEvent("SAlocalchat.playSound", true)
addEventHandler("SAlocalchat.playSound", root, play)

function income(message)
	addText(source,message)
end
addEvent("SAlocalchat.addBubble", true)
addEventHandler("SAlocalchat.addBubble", root, income)
			...
			if isElement(v[1]) then
				if getElementHealth(v[1]) > 0 then
					...
					if posx and distance <= 30 and ( isLineOfSightClear(cx,cy,cz,px,py,pz,true,true,false,true,false,true,true,elementtoignore1) or isLineOfSightClear(cx,cy,cz,px,py,pz,true,true,false,true,false,true,true,elementtoignore2) ) and ( not maxbubbles or  v[3] < maxbubbles ) then -- change this when multiple ignored elements can be specified
						...
						dxDrawRectangle(posx - (3 + (0.5 * width)),posy - (2 + (v[3] * 20)),width + 5,19,tocolor(25,25,25,100))
						dxDrawRectangle(posx - (6 + (0.5 * width)),posy - (2 + (v[3] * 20)),width + 11,19,tocolor(25,25,25,100))
						dxDrawRectangle(posx - (8 + (0.5 * width)),posy - (1 + (v[3] * 20)),width + 15,17,tocolor(25,25,25,100))
						dxDrawRectangle(posx - (10 + (0.5 * width)),posy - (1 + (v[3] * 20)),width + 19,17,tocolor(25,25,25,100))
						dxDrawRectangle(posx - (10 + (0.5 * width)),posy - (v[3] * 20) + 1,width + 19,13,tocolor(25,25,25,100))
						dxDrawRectangle(posx - (12 + (0.5 * width)),posy - (v[3] * 20) + 1,width + 23,13,tocolor(25,25,25,100))
						dxDrawRectangle(posx - (12 + (0.5 * width)),posy - (v[3] * 20) + 4,width + 23,7,tocolor(25,25,25,100))
